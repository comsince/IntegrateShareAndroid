...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script>
		  ...
		  function doSetting(account,net)
		  {
		      ...
		      $.post("<%=basePath%>/admin/user_modify.action", {account:account,net:net},
					function(data){
	...
	<body class="web-app ui-selectable">
		...
		<div id="mainWrapper">
			<div class="panel panel-default">
				...
				<div class="panel-body" style="padding: 5px;">
					<form action="<%=basePath%>/admin/user_netManage.action" method="post"
								...
								<c:forEach var="user" items="${page.dataList}">
								   <c:if test="${user.account ne 'admin'}">
									<tr id="${user.account}">
										...
										<td>
										   <c:if test="${user.net eq null || user.net eq '0'}">
										      <span class="label label-success">已开启</span>
										   </c:if>
										   
										   <c:if test="${user.net eq  '1'}">
		...
		<script>
		       $('#netManageMenu').addClass('current');
